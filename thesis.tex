...
\chapter{Adaptivität von bestehenden Web Components}
	...
	\section{Anpassung der Web Components}
		\subsection{google-map}
			...
			\subsubsection{Einrichtung des Google Map Web Component}
			Um das \emph{Web Component} zu installieren wurde Bower genutzt. \emph{Bower} ist ein Paketverwartungsprogramm, welches das Aktualisieren und Installieren von Bibliotheken und Frameworks verwendet. Dabei wird \emph{Bower} als ein Kommandozeilenprogramm genutzt.\cite[vgl.]{bower} Hier liegt das \emph{Bower} \emph{Web Component} in der Version \enquote{GoogleWebComponents/google-map\#\^1.1.3} vor.
			Wenn das \emph{Web Component} eingebunden ist, kann es in einer unter dem \ac{HTML} Tag \enquote{<google-map>} genutzt werden. Dieses muss jedoch mit einem \emph{Google} \ac{API} Schlüssel, unter dem Namen \enquote{Google Maps JavaScript API} instanziiert werden. Dieser wurde über ein \emph{Google} Konto generiert.
